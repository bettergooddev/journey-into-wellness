...
export const OfferingRowLabel = () => {
  const { data, rowNumber: rowNumberFromPayload } = useRowLabel<NonNullable<OfferingsListBlock['pitches']>[number]>()

  const offeringId = data?.offering || ''

  const [{ data: offeringData, isError, isLoading }, { setParams }] = usePayloadAPI(`/api/offerings`, {
    initialParams: {
      depth: 0,
